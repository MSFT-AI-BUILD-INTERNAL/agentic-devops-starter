...
| `THIRDPARTY_GITHUB_PAT` | Yes (Anthropic) | unset | PAT used to authenticate the Copilot SDK session for `POST /v1/messages` (Anthropic-compatible endpoint for third-party clients like Claude Code). This stands in for the GitHub Apps OAuth token the browser flow would normally supply |
