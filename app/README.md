...
| `THIRDPARTY_GITHUB_PAT` | No | unset | PAT used to authenticate the Copilot SDK session for `GET /v1/models` / `POST /v1/messages` (Anthropic-compatible endpoints for third-party clients like Claude Code). These endpoints require no request auth, so this PAT stands in for the GitHub Apps OAuth token the browser flow would normally supply |
