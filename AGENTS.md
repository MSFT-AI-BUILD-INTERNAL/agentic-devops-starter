...
All code Copilot writes or modifies in this repository. This harness encodes general behavioral guidelines to reduce common LLM coding mistakes.

### Priority

**This harness takes precedence.** Where its rules overlap with or contradict any other harness in this file (including `project-governance`, `code-review-criteria`, and `code-generation-evaluation`), the rules below win.
...
> **Interaction with other harnesses:** This rule overrides the broader "Dead Code Elimination" guidance in `project-governance`. Do not delete pre-existing dead code as part of an unrelated change; surface it to the user instead. Dead-code removal is in scope only when it is the explicit task or when your own changes orphaned the code.
